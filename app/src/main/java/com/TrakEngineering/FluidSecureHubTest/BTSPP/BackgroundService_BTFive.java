...
public class BackgroundService_BTFive extends Service {
    ...
    public void ProceedToPostTransactionCommands() {
        ...
        if (CommonUtils.checkBTVersionCompatibility(versionNumberOfLinkFive, BTConstants.supportedLinkVersionForLast20) && IsEleventhTransaction.equalsIgnoreCase("true")) { // Last20 command supported from this version onwards
            last20Command();
        } else if (CommonUtils.checkBTVersionCompatibility(versionNumberOfLinkFive, BTConstants.supportedLinkVersionForBypassPumpReset)) { // Bypass pump reset command supported from this version onwards
    ...
    private void ResetEleventhTransactionFlag() {
        try {
            ...
            String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BackgroundService_BTFive.this) + ":" + userEmail + ":" + "SetEleventhTransaction" + AppConstants.LANG_PARAM);

            EleventhTransaction eleventhTransaction = new EleventhTransaction();
            eleventhTransaction.SiteId = BTConstants.BT5SITE_ID;
            ...
            pref = BackgroundService_BTFive.this.getSharedPreferences("storeEleventhTransactionFlag5", 0);
            editor = pref.edit();
            ...
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink_5: ResetEleventhTransactionFlag Exception: " + ex.getMessage());
        }
    }
}
