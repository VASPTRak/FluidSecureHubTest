...
public class BackgroundService_BTFive extends Service {
    ...
    public void ContinueToNextCommand() {
        if (versionNumberOfLinkFive == 148) { // Bypass pump reset supported in this version
        ...
        try {
            if (IsBypassPumpReset != null) {
                if (IsBypassPumpReset.trim().equalsIgnoreCase("True") && !CommonUtils.CheckDataStoredInSharedPref(BackgroundService_BTFive.this, "storeBypassPumpResetFlag5")) {
                    ...
                    if (IsThisBTTrnx) {
                        if (AppConstants.GenerateLogs)
                            AppConstants.WriteinFile(TAG + " BTLink 5: Sending bypass pump reset command to Link: " + LinkName);
                        BTSPPMain btspp = new BTSPPMain();
                        ...
                        public void onTick(long millisUntilFinished) {
                            ...
                            if (attempt > 0) {
                                if (Request.contains(BTConstants.bypass_pump_reset_command) && Response.contains("rm_delay_time")) {
                                    if (AppConstants.GenerateLogs)
                                        AppConstants.WriteinFile(TAG + " BTLink 5: Checking bypass pump reset command response:>> " + Response.trim());
                                    UpdateBypassPumpResetFlagForLink();
                                    transactionIdCommand(TransactionId); // Continue to transactionId Command
                                    cancel();
                                } else {
                                    if (AppConstants.GenerateLogs)
                                        AppConstants.WriteinFile(TAG + " BTLink 5: Checking bypass pump reset command response. Response: false");
                                }
                            }
                        }
            ...
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 5: BypassPumpResetCommand Exception:>>" + e.getMessage());
            transactionIdCommand(TransactionId); // Continue to transactionId Command
        }
    }
    ...
    private void UpdateBypassPumpResetFlagForLink() {
        try {
            ...
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 5: UpdateBypassPumpResetFlagForLink Exception: " + ex.getMessage());
        }
    }
