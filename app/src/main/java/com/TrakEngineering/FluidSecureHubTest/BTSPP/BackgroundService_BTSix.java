...
public class BackgroundService_BTSix extends Service {
            ...
            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                ...
                public void run() {
                    if (BTConstants.isHotspotDisabled) {
                        //Enable Hotspot
                        com.TrakEngineering.FluidSecureHubTest.WifiHotspot.WifiApManager wifiApManager = new com.TrakEngineering.FluidSecureHubTest.WifiHotspot.WifiApManager(BackgroundService_BTSix.this);
            ...
            try {
                ...
                if (Action.equalsIgnoreCase("BlueLinkSix")) {
                    ...
                    if (BTConstants.forOscilloscope) {
                        ...
                        //AppConstants.WriteinFile(TAG + " BTLink 6: onReceive Response:" + Response.trim());
    ...
    private void InsertInitialTransactionToSqlite() {
        ...
        String userEmail = CommonUtils.getCustomerDetails_backgroundServiceBT(BackgroundService_BTSix.this).PersonEmail;
        String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BackgroundService_BTSix.this) + ":" + userEmail + ":" + "TransactionComplete");
        ...
        try {
            if (cd.isConnectingToInternet()) {
                ...
                if (hoseid != null && !hoseid.trim().isEmpty()) {
                    new BackgroundService_BTSix.UpgradeCurrentVersionWithUpgradableVersion(objEntityClass).execute();
                }
                //=============================================================
    ...
    private void ReadScope() {
        if (AppConstants.GenerateLogs)
            AppConstants.WriteinFile(TAG + " BTLink 6: ReadScope started.");
        readScopeLoop_on = true;
        timerBtScope = new Timer();
        TimerList_ReadpulseBT6.add(timerBtScope);
        TimerTask tt = new TimerTask() {
            @RequiresApi(api = Build.VERSION_CODES.P)
            @Override
            public void run() {
                ...
                if (scopeCounter > 1000) {
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink 6: Readings >> " + BTConstants.BTLinkVoltageReadings.size());
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink 6: ReadScope end.");
                    BTConstants.ScopeStatus = "DONE";
                ...
                if (BTConstants.TerminateReadingProcess) {
                    if (AppConstants.GenerateLogs)
                        AppConstants.WriteinFile(TAG + " BTLink 6: Terminate Reading Process.");
                    BTConstants.ScopeStatus = "";
                    BTConstants.ReadingProcessComplete = true;
                    scopeCounter = 0;
                    cancel();
    ...
    private void StopScopeReading() {
        ...
        try {
            ...
            e.printStackTrace();
            if (AppConstants.GenerateLogs)
                AppConstants.WriteinFile(TAG + " BTLink 6: StopScopeReading Exception:>>" + e.getMessage());
        }
    }
}
