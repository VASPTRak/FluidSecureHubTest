...
public class BackgroundService_BTThree extends Service {
    ...
    public void proceedToPostTransactionCommands() {
        ...
        if (CommonUtils.checkBTVersionCompatibility(versionNumberOfLinkThree, BTConstants.SUPPORTED_LINK_VERSION_FOR_LAST20) && isEleventhTransaction.equalsIgnoreCase("true")) { // Last20 command supported from this version onwards
            last20Command();
        } else if (CommonUtils.checkBTVersionCompatibility(versionNumberOfLinkThree, BTConstants.SUPPORTED_LINK_VERSION_FOR_BYPASS_PUMP_RESET)) { // Bypass pump reset command supported from this version onwards
        ...
        try {
            if (IsBypassPumpReset != null) {
                if (IsBypassPumpReset.trim().equalsIgnoreCase("True") && !CommonUtils.CheckDataStoredInSharedPref(BackgroundService_BTThree.this, "storeBypassPumpResetFlag3")) {
                    ...
                    if (IsThisBTTrnx) {
                        if (AppConstants.GENERATE_LOGS)
                            AppConstants.writeInFile(TAG + " BTLink_3: Sending bypass pump reset command to Link: " + LinkName);
                        BTSPPMain btspp = new BTSPPMain();
                        ...
                        public void onTick(long millisUntilFinished) {
                            ...
                            if (attempt > 0) {
                                if (Request.contains(BTConstants.BYPASS_PUMP_RESET_COMMAND) && Response.contains("rm_delay_time")) {
                                    if (AppConstants.GENERATE_LOGS)
                                        AppConstants.writeInFile(TAG + " BTLink_3: Checking bypass pump reset command response:>> " + Response.trim());
                                    updateBypassPumpResetFlagForLink();
                                    proceedToNextCommand();
                                    cancel();
                                } else {
                                    if (AppConstants.GENERATE_LOGS)
                                        AppConstants.writeInFile(TAG + " BTLink_3: Checking bypass pump reset command response. Response: false");
                                }
                            }
                        }
            ...
            e.printStackTrace();
            if (AppConstants.GENERATE_LOGS)
                AppConstants.writeInFile(TAG + " BTLink_3: Bypass Pump Reset Command Exception:>>" + e.getMessage());
            proceedToNextCommand();
        }
    }
    //endregion
    ...
    private void updateBypassPumpResetFlagForLink() {
        try {
            ...
            if (AppConstants.GENERATE_LOGS)
                AppConstants.writeInFile(TAG + " BTLink_3: updateBypassPumpResetFlagForLink Exception: " + ex.getMessage());
        }
    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            try {
                ...
                if (Action.equalsIgnoreCase("BlueLinkThree")) {
                    ...
                    //if (AppConstants.GENERATE_LOGS)AppConstants.writeInFile(TAG + " BTLink_3: Link Response>>" + Response);
    ...
    private void resetEleventhTransactionFlag() {
        try {
            ...
            String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BackgroundService_BTThree.this) + ":" + userEmail + ":" + "SetEleventhTransaction" + AppConstants.LANG_PARAM);

            EleventhTransaction eleventhTransaction = new EleventhTransaction();
            eleventhTransaction.SiteId = BTConstants.BT3SITE_ID;
            ...
            pref = BackgroundService_BTThree.this.getSharedPreferences("storeEleventhTransactionFlag3", 0);
            editor = pref.edit();
            ...
            if (AppConstants.GENERATE_LOGS)
                AppConstants.writeInFile(TAG + " BTLink_3: resetEleventhTransactionFlag Exception: " + ex.getMessage());
        }
    }
}
