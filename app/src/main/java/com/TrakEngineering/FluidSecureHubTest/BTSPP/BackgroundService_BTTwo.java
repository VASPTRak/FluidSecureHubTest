...
public class BackgroundService_BTTwo extends Service {
    ...
    public void proceedToPostTransactionCommands() {
        ...
        if (CommonUtils.checkBTVersionCompatibility(versionNumberOfLinkTwo, BTConstants.SUPPORTED_LINK_VERSION_FOR_LAST20) && IsEleventhTransaction.equalsIgnoreCase("true")) { // Last20 command supported from this version onwards
            last20Command();
        } else if (CommonUtils.checkBTVersionCompatibility(versionNumberOfLinkTwo, BTConstants.SUPPORTED_LINK_VERSION_FOR_BYPASS_PUMP_RESET)) { // Bypass pump reset command supported from this version onwards
    ...
    private void bypassPumpResetCommand() {
        try {
            if (IsBypassPumpReset != null) {
                if (IsBypassPumpReset.trim().equalsIgnoreCase("True") && !CommonUtils.CheckDataStoredInSharedPref(BackgroundService_BTTwo.this, "storeBypassPumpResetFlag2")) {
                    ...
                    if (IsThisBTTrnx) {
                        if (AppConstants.GENERATE_LOGS)
                            AppConstants.writeInFile(TAG + " BTLink_2: Sending bypass pump reset command to Link: " + LinkName);
                        BTSPPMain btspp = new BTSPPMain();
                        btspp.send2(BTConstants.BYPASS_PUMP_RESET_COMMAND);
                    }
                        ...
                        public void onTick(long millisUntilFinished) {
                            ...
                            if (attempt > 0) {
                                if (Request.contains(BTConstants.BYPASS_PUMP_RESET_COMMAND) && Response.contains("rm_delay_time")) {
                                    if (AppConstants.GENERATE_LOGS)
                                        AppConstants.writeInFile(TAG + " BTLink_2: Checking bypass pump reset command response:>> " + Response.trim());
                                    updateBypassPumpResetFlagForLink();
                                    proceedToNextCommand();
                                    cancel();
                                } else {
                                    if (AppConstants.GENERATE_LOGS)
                                        AppConstants.writeInFile(TAG + " BTLink_2: Checking bypass pump reset command response. Response: false");
                                }
                            }
                        }
            ...
            e.printStackTrace();
            if (AppConstants.GENERATE_LOGS)
                AppConstants.writeInFile(TAG + " BTLink_2: Bypass Pump Reset Command Exception:>>" + e.getMessage());
            proceedToNextCommand();
        }
    }
    //endregion
    ...
    private void updateBypassPumpResetFlagForLink() {
        try {
            ...
            String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BackgroundService_BTTwo.this) + ":" + userEmail + ":" + "UpdateBypassPumpResetFlagForLink" + AppConstants.LANG_PARAM);

            BypassPumpResetEntity bypassPumpReset = new BypassPumpResetEntity();
            bypassPumpReset.SiteId = BTConstants.BT2SITE_ID;
            bypassPumpReset.IsBypassPumpReset = "False";

            Gson gson = new Gson();
            String jsonData = gson.toJson(bypassPumpReset);

            storeBypassPumpResetFlag(BackgroundService_BTTwo.this, jsonData, authString);
        } catch (Exception ex) {
            if (AppConstants.GENERATE_LOGS)
                AppConstants.writeInFile(TAG + " BTLink_2: updateBypassPumpResetFlagForLink Exception: " + ex.getMessage());
        }
    }
    ...
    public void storeBypassPumpResetFlag(Context context, String jsonData, String authString) {
        try {
            ...
            pref = context.getSharedPreferences("storeBypassPumpResetFlag2", 0);
            editor = pref.edit();
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            try {
                ...
                if (Action.equalsIgnoreCase("BlueLinkTwo")) {
                    ...
                    //if (AppConstants.GENERATE_LOGS)AppConstants.writeInFile(TAG + " BTLink_2: Link Response>>" + Response);
    ...
    private void resetEleventhTransactionFlag() {
        try {
            ...
            String authString = "Basic " + AppConstants.convertStingToBase64(AppConstants.getIMEI(BackgroundService_BTTwo.this) + ":" + userEmail + ":" + "SetEleventhTransaction" + AppConstants.LANG_PARAM);

            EleventhTransaction eleventhTransaction = new EleventhTransaction();
            eleventhTransaction.SiteId = BTConstants.BT2SITE_ID;
            ...
            pref = BackgroundService_BTTwo.this.getSharedPreferences("storeEleventhTransactionFlag2", 0);
            editor = pref.edit();
            ...
            if (AppConstants.GENERATE_LOGS)
                AppConstants.writeInFile(TAG + " BTLink_2: resetEleventhTransactionFlag Exception: " + ex.getMessage());
        }
    }
}
